...
Start by loading the `aae.data` package and downloading some data to work with.
...
We're often comparing flow among rivers. A straight metric (e.g., the maximum) might not be useful or comparable in several rivers of different size or structure. The `calculate` function addresses this with a rescale argument, which provides a flexible way to rescale your calculated metric by some other aspect of the flow (e.g. the long-term median flow). The rescale argument looks very similar to the resolution argument.

```{r calculate-rescale-examples}
...
spring_flow_standardised <- calculate(
  ...
  date = flow_405232$date_formatted,
  resolution = survey(season = 9:11, lag = 0, subset = 2009:2015),
  rescale = by_median(subset = 2004:2020, season = 1:12, na.rm = TRUE),
  na.rm = TRUE
)
```

This might look like a fair bit of typing, but the `rescale` argument has some defaults built-in, so you don't need to type everything out unless you want to do something different. The current defaults are `subset = all_available_years`, `fun = median` and `season = 1:12`.
